...
Subject: update of allout.el (extensive outline mode)
Organization: Nat'l Institute of Standards and Technology
Distribution: gnu
Date: Sun, 18 Apr 1993 21:05:37 GMT

* This is a long overdue update of my extended outline-mode code,
  allout.el.  I was hoping to have a few things more polished up, but
  i could hold off forever before i'd be satisfied.  There's a lot of
  good stuff here, including a few new (and hopefully more than just
  spiffy) features, as well as numerous bug fixes and refinements.

  While the function docstrings and commenting is ever improving, i
  continue to regret that that's all i can provide in the way of
  documentation.

*                           New Features

.* Configurable topic-prefix leading character(s), including
   program language comment chars.  This enables outline
   structuring and manipulation of your program code.

.* Open-topic does a better job of tailoring the white-space
   around topics that it produces to suit the context of the topic
   being created.

.* Reduced the set of standard bullets to a more suitable size.
   At the same time, reduced the constraints on what can be a
   topic bullet, allowing even more distinctive bullets.

.* Accomodated newer versions of isearch-mode, without abandoning old
   version.  (This version fixes an former incompatability problem
   with the newer isearch-mode code.)

.* hide-current-children, which normally closes the children
   of the current topic, closes the topic itself if its children
   are already closed (or non-existent).  Makes it easy to
   progressively close down a subtree, from the inside outwards.

.* Topic navigation commands put the cursor just after the topic
   prefix, ready for action on the body.

.* Can change exposure of outlines even in read-only buffers.

.* numerous bug fixes and code improvements (including some further
   weaning from dependence on cl.el).

.* Some keybinding shifts - you'll have to check the outline-mode
   docstring.

*                      General Allout Features

.* Backwards support for old-format outlines.  (Set

.* Informative topic prefixes

.* More informative bullet structure
ome examples:

.* Structurally-based navigation and editing
including pasting of topics to differnt depths than that from

.* Designable topic prefix lead-characters, allows making

.* Dynamic exposure during autosearch (requires elisp-archives

.* Automatically maintained hanging indentation on topic bodies.

.* Invocable file cross-references

.* More systematized, optimized, and documented outline

*                                 Cheers,

Ken Manheimer
klm@nist.gov  301 975-3539
NIST Shared File Service Developer and Manager, or something like that.
...
;; |22-Dec-1992|$Id: outline-mode.el,v 1.1.1.1 1996/10/09 01:12:30 fox Exp $||
